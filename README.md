...
<p align="center">
An automated fuzzer and harness generator for Ada, C, C++, Rust, Java, Python, Perl, and Go —
...
- **Eight languages, one engine.** Ada, C, C++, Rust, Java, Python, Perl, and Go are peer
  first-class lanes over a shared coverage-guided fork-server engine — no cargo-fuzz, Jazzer,
  Atheris, or `go test -fuzz` required (AFL++ is an optional adapter for native C/C++).
