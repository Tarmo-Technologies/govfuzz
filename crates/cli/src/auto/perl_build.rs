...
fn locate_perl_runtime() -> Option<PathBuf> {
    if let Ok(exe) = std::env::current_exe() {
        ...
        for _ in 0..6 {
            if let Some(d) = &dir {
                let cand = d.join("perl_runtime");
                if cand.join("govfuzz_driver.pl").is_file() {
                    return Some(cand);
                }
    ...
    let from_manifest = Path::new(env!("CARGO_MANIFEST_DIR"))
        ...
        .map(|root| root.join("perl_runtime"));
    if let Some(p) = &from_manifest {
        if p.join("govfuzz_driver.pl").is_file() {
...
pub fn build_perl_harness(
    ...
    match smoke {
        ...
        Ok(out) => {
            return PerlBuildResult::Failed {
                reason: format!(
                    "target module `{}` is not loadable (skipped cleanly): {}",
                    sub.package,
                    String::from_utf8_lossy(&out.stderr)
                        .lines()
                        .last()
...
mod tests {
    ...
    fn harness_loads_target_and_calls_function() {
        let h = generate_harness(Path::new("/proj/lib/My/P.pm"), "My::P::parse($data)");
