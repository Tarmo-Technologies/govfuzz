...
fn generate_harness(module: &str, func: &PyFunction, receiver: Option<&str>) -> String {
    ...
    format!(
        "# SPDX-License-Identifier: Apache-2.0\n\
         ...
         {import}\
         \n\
         _govfuzz_target_entered = False\n\
         \n\
         def _govfuzz_mark_target_entry():\n\
         \x20   global _govfuzz_target_entered\n\
         \x20   if _govfuzz_target_entered:\n\
         \x20       return\n\
         \x20   path = os.environ.get(\"GOVFUZZ_TARGET_ENTRY_SHM\")\n\
         \x20   if not path:\n\
         \x20       return\n\
         \x20   try:\n\
         \x20       with open(path, \"wb\") as entry:\n\
         \x20           entry.write(b\"\\x01\")\n\
         \x20       _govfuzz_target_entered = True\n\
         \x20   except OSError:\n\
         \x20       pass\n\
         \n\
         \n\
         ...
         {lines}    _govfuzz_mark_target_entry()\n\
         {call}",
        import = import,
        lines = lines,
        call = call,
...
mod tests {
    ...
    fn a_module_name_that_is_not_an_identifier_is_imported_by_string() {
        ...
        // syntax" — a govfuzz bug reported as if the project's code were broken.
    ...
    fn single_bytes_param_gets_whole_input() {
        ...
        assert!(
            h.contains("    _govfuzz_mark_target_entry()\n    _t.parse(a0)"),
