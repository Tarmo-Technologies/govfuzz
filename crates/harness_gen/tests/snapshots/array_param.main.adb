...
begin
   loop
      ...
      AdaFuzz.Probe.Set_Target (16#000B#);
      begin
      declare
         Cur : AdaFuzz.Decode.Cursor := AdaFuzz.Decode.Open (Buf'Unchecked_Access, Last);
         function Decode_Items return Int_Array is
            Len_Items : constant Natural := AdaFuzz.Decode.Bounded_Length (Cur, 0, 16);
            Tmp_Items : Int_Array (1 .. Len_Items);
         begin
            for I in Tmp_Items'Range loop
               Tmp_Items (I) := Integer (AdaFuzz.Decode.I32 (Cur));
            end loop;
            return Tmp_Items;
         end Decode_Items;
