...
//! (Anthropic, OpenAI, local Ollama) land under the
...
/// downstream crates (e.g. a feature-gated anthropic_provider).
...
/// wrapping `claude`, `ollama run <model>`, or any local CLI that
...
/// CLI (claude, ollama against a slow model) since the retry loop in
