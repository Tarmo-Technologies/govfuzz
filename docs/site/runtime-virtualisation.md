...
| `unlink`, `unlinkat`, `remove` | Audit only. Logs audited successful file deletion operations so executable oracles can flag parent-directory delete paths. |
| `stat`, `fstat`, `lstat`, `access`, `faccessat` | Return a synthesised 0-byte regular-file stat for the same allow-list. `access`/`faccessat` also log a `path_check` event on success (the TOCTOU time-of-check) so executable oracles can flag a path checked then opened. |
| `readlink`, `readlinkat` | Return a synthesised relative target under `<work>/auto/fake_fs/…`. |
| `connect` (AF_UNIX, AF_INET, AF_INET6) | On ECONNREFUSED, replace the fd with one end of a `socketpair()`; the peer end is held by the shim. |
...
| `gethostbyname`, `getaddrinfo` | On EAI_NONAME / NULL return, synthesise a struct pointing at `127.0.0.1` so the followup `connect` lands on a fake socket. |
...
| `system`, `popen` | Audit only. Logs shell command strings so executable oracles can flag metacharacter-driven command injection. |
